...
public class CasClient {
	...
	public String login (String serviceUrl, String username, String password) throws CasAuthenticationException, CasProtocolException
	{
		...
		System.out.println(casBaseURL + CAS_LOGIN_URL_PART + CAS_SERVICE_URL_PART + serviceUrl);
		...
		if (lt == null)
		{
			if(MyDebug.DEBUG) System.out.println("Cannot retrieve LT from CAS. Aborting authentication for '" + username + "'");
		...
		else
		{
			...
			try
			{
				...
				if(MyDebug.DEBUG) System.out.println( "POST RESPONSE STATUS=" + response.getStatusLine().getStatusCode() + " : " + response.getStatusLine().toString());
				...
				if(!validLogin) {
					if(MyDebug.DEBUG) System.out.println( "Cas did not return a TGC. Wrong username/password combination?");
	...
	private TokenContainer getLTFromLoginForm (String serviceUrl, HttpContext context)
	{
		...
		try
		{
			...
			if(MyDebug.DEBUG) System.out.println( "Ready to get our LT from " + casBaseURL + CAS_LOGIN_URL_PART + "?service=" + serviceUrl);
			...
			{
				if(MyDebug.DEBUG) System.out.println("Could not obtain LT token from CAS: " + response.getStatusLine().getStatusCode() + " / " + response.getStatusLine());
			}
			else
			{
				HttpEntity entity = response.getEntity();

				System.out.println("____ RESULT _____");
