...
export class CourseResultsService {
   ...
   */
  getRequirementCourseResults(
    cid: number
  ): Observable<EvaluationUserResults[]> {
    return this.http.get<EvaluationUserResults[]>(
      `/api/v1/courses/${cid}/evaluation/results`
    );
  }
  ...
  getRequirementResultData(): {
    ...
    id: number;
    tasks: {
      task: { name: string; mediaType: string; deadline: string };
      passed: boolean;
      attempts: number;
    };
  }[] {
  ...
  getStudentResults(cid: number): Observable<CourseResult[]> {
    return this.http.get<CourseResult[]>(
      `/api/v1/courses/${cid}/results/student`
    );
  }
}
