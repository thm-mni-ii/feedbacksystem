...
class TestsystemService {
  ...
  def updateTestsystem(id_string: String, jsonNode: JsonNode): Boolean = {
    ...
    val settings: List[String] = if (settingNode != null) nodeIteratorToList[String](settingNode.iterator()) else List()
    ...
    var ok = true
    if (name != null) {
      DB.update("update testsystem set name = ? where testsystem_id = ?", name, id_string)
    }
    if (description != null) {
