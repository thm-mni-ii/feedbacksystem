...
class CourseService {
  ...
  def getAll(ignoreHidden: Boolean = true): List[Course] = DB.query(
    "SELECT course_id, name, description, visible FROM course" + (if (ignoreHidden) " WHERE visible = 1" else ""),
  ...
  def findByPattern(pattern: String, ignoreHidden: Boolean = true): List[Course] = DB.query(
    "SELECT course_id, name, description, visible FROM course WHERE name like ?" + (if (ignoreHidden) " AND visible = 1" else ""),
  ...
  def find(id: Int): Option[Course] = DB.query(
    "SELECT course_id, name, description, visible FROM course WHERE course_id = ?",
  ...
  def update(cid: Int, course: Course): Boolean = {
    1 == DB.update("UPDATE course SET name = ?, description = ?, visible = ? WHERE course_id = ?",
      course.name, course.description, course.visible, cid)
  }
