...
function toStudyQuestion(question: QuestionMockItem): Question {
  if (!question._id) {
  ...
  const isFillInTheBlanks = question.questiontype === 'fill-in-the-blank'
  ...
  const questionConfiguration = isFillInTheBlanks
    ? {
        showBlanks: question.showBlanks ?? true,
        textParts: question.textParts ?? [{ order: 1, text: '', isBlank: false }]
      }
    : isMatching
      ? {
          categories: (question.answerColumns ?? []).map((column) => ({
            id: String(column.id),
            label: column.name
          })),
          items: (question.optionRows ?? []).map((row) => ({
            id: String(row.id),
            text: row.text,
...
export const questions: Question[] = questionMocks.map(toStudyQuestion)
