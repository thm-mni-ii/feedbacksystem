...
export interface CompetencyGraphEdgeData {
  source: string
  target: string
  color?: string
  width?: number
...
export function useCompetencyGraph(mockCompetencies: Competency[], mockQuestions: Question[]) {
  // ─── State ────────────────────────────────────────────────────────────
  ...
  const getCompetencyColor = (comp?: Competency): string => {
    if (!comp) return competencyGraphPalette.chipFallback
    return competencyGraphPalette.accent
  }
  ...
  // ─── Graph Nodes ──────────────────────────────────────────────────────
  const graphNodes = computed<Record<string, CompetencyGraphNodeData>>(() => {
    const nodes: Record<string, CompetencyGraphNodeData> = {}

    nodes[course.value.id] = {
      name: course.value.name,
      type: 'course',
      color: competencyGraphPalette.courseNode,
      radius: 17,
      data: course.value
    }
    ...
    questions.value.forEach((q) => {
      ...
      nodes[q.id] = {
        name: short,
        type: 'question',
        color: competencyGraphPalette.questionNode,
        radius: 5,
        data: q
      }
    })
  ...
  // ─── Graph Edges ──────────────────────────────────────────────────────
  const graphEdges = computed<Record<string, CompetencyGraphEdgeData>>(() => {
    const edges: Record<string, CompetencyGraphEdgeData> = {}
    ...
    competencies.value
      ...
      .forEach((c) => {
        edges[`e-course-${c.id}`] = {
          ...
          target: c.id,
          color: competencyGraphPalette.edgeFocus,
          width: 2
        }
      })
    ...
    questions.value.forEach((q) => {
      ...
      qCompIds.forEach((competencyId) => {
        edges[`e-${competencyId}-${q.id}`] = {
          source: competencyId,
          target: q.id,
          color: competencyGraphPalette.edgeDefault,
          width: 1
        }
      })
    })
  ...
  // ─── Graph Configs ────────────────────────────────────────────────────
  const configs = vNG.defineConfigs<CompetencyGraphNodeData, CompetencyGraphEdgeData>({
    view: {
    ...
    node: {
      ...
      label: {
        ...
        background: {
          visible: true,
          color: competencyGraphPalette.labelBackground,
          padding: {
            vertical: 2,
            horizontal: 6
          },
        ...
        color: competencyGraphPalette.textPrimary
      }
    },
    edge: {
      normal: {
        color: (e) => e.color ?? competencyGraphPalette.edgeDefault,
        width: (e) => e.width ?? 1.5
      }
    }
  })
  ...
  /**
   * Legt eine neue lokale Kompetenz an oder aktualisiert eine bestehende.
   * Wird vom `DialogEditCompetency` (mit `persist: false`) genutzt, da der
   * Kompetenzgraph-Competencies werden bisher nur lokal (Mock-Daten) verwaltet.
   */
  const upsertCompetency = (updatedCompetency: Competency) => {
    const exists = competencies.value.some((c) => c.id === updatedCompetency.id)
    competencies.value = exists
      ? competencies.value.map((c) => (c.id === updatedCompetency.id ? updatedCompetency : c))
      : [...competencies.value, updatedCompetency]
  }
