  ...
  <div class="cyto-container">
    ...
    <div class="graph-header">
      ...
      <p class="instructions">
        Klicken Sie auf die Knoten oder Verbindungen, um Änderungen vorzunehmen
      </p>
    </div>
    ...
    <div v-if="showModalNum" class="modal-overlay">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" @click="validateAndChangeScore" :disabled="!nodeData">
  ...
  try {
    if (id.questionId !== 'new') {
      ...
      if (data.data.isEmpty) {
        ...
        const prevData = await catalogService.getPreviousQuestion(
          id.catalogId as string,
          data.data._id
        )
    ...
    cy.value = cytoscape({
      ...
      elements: [
        {
          data: { id: 'center', label: questionText },
          position: { x: 400, y: 0 },
          grabbable: false
        },
        {
          data: { id: 'left', label: prevText, hiddenData: prevId, hidden: buttonsHidden },
          position: { x: 200, y: 0 },
          grabbable: false
        },
        { data: { source: 'left', target: 'center', hidden: buttonsHidden } },
        {
          data: { id: 'correct', label: maxKey, hiddenData: maxId, hidden: buttonsHidden },
          position: { x: 650, y: -120 },
          grabbable: false
        },
        {
          data: { source: 'center', target: 'correct', label: maxKeyNumber, hidden: buttonsHidden },
          grabbable: false
        },
        {
          data: { id: 'partial', label: midKey, hiddenData: midId, hidden: buttonsHidden },
          position: { x: 650, y: 0 },
          grabbable: false
        },
        {
        ...
        {
          data: { id: 'incorrect', label: minKey, hiddenData: minId, hidden: buttonsHidden },
          position: { x: 650, y: 120 },
          grabbable: false
        },
        {
        ...
        {
          data: { id: 'invisible', label: '', hidden: 'true' },
          position: { x: 650, y: 60 },
          grabbable: false
        }
      ],
    ...
    cy.value.on('tap', 'node', async (event) => {
      ...
      if (clickedNode.data('label') === '+') {
        ...
        const showInput = !(
          clickedNode.id() === 'partial' ||
          firstQuestion.value ||
          currentQuestion.value === 'new'
        )
        ...
        clickedNode.data('hiddenData') !== null &&
        clickedNode.data('hiddenData') !== undefined
      ) {
...
const attachButtonToExistingNode = (nodeId: string) => {
  ...
  if (
    (nodeId === 'correct' || nodeId === 'partial' || nodeId === 'incorrect') &&
    node.data('label') !== '+'
  ) {
