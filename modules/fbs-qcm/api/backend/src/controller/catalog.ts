import {
  addNewChildrenToQuestion,
  catalogScore,
  changeScoreNeededForQuestion,
  createSingleCatalog,
  deleteSingleCatalog,
  editCatalogInformation,
  editSingleCatalog,
  emptyCatalogInformation,
  getAllCatalogs,
  getPreviousQuestionInCatalog,
  getSingleCatalog,
...
const putCatalog = async (req: Request, res: Response) => {
  try {
    ...
    if (req.user !== undefined) {
      ...
      const data = await editSingleCatalog(
        catalogId,
        token,
        requestData,
        req.user,
...
const postCatalog = async (req: Request, res: Response) => {
  try {
    ...
    if (req.user !== undefined) {
      ...
      const course = requestData.course;
      delete requestData.course;
      const data = await createSingleCatalog(
        requestData,
        token,
        req.user,
        course
...
const addChildrenToQuestion = async (req: Request, res: Response) => {
  try {
    ...
    if (req.user !== undefined) {
      ...
      const result = await addNewChildrenToQuestion(
        req.user,
        questionId,
        children,
        key,
...
const editCatalog = async (req: Request, res: Response) => {
  try {
    ...
    if (req.user !== undefined) {
      const data = await editCatalogInformation(
        req.user,
        catalogId,
        questionId
      );
...
const getPreviousQuestion = async (req: Request, res: Response) => {
  try {
    ...
    const catalogId = req.params.catalog as string;
    if (req.user !== undefined) {
      const data = await getPreviousQuestionInCatalog(
        req.user,
        catalogId,
        questionId
      );
...
const changeNeededScore = async (req: Request, res: Response) => {
  try {
    ...
    if (req.user !== undefined) {
      ...
      const result = await changeScoreNeededForQuestion(
        req.user,
        questionId,
        score,
        transition
