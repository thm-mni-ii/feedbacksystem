...
class QueryProcessorQueryTestBast(QueryProcessorTestBase):
    def _query_test_case(
        ...
        res = qb.process(Submission.new_solution(solution_query, course_id=course_id, task_id=task_id))
        errors = []
        ...
        res = qb.process(Submission(submission_query, course_id=course_id, task_id=task_id))
...
class QueryProcessorMinimalQueryTest(QueryProcessorQueryTestBast):
    def test_correct(self):
        self._query_test_case("SELECT username, mail, password FROM user WHERE registration_date > 100;",
    ...
    def test_wrong_join(self):
        self._query_test_case("SELECT username, mail, password FROM user JOIN submissions ON user.id = submission.user_id WHERE registration_date > 100;",
    ...
    def test_wrong_group_by(self):
        self._query_test_case("SELECT username, mail, password FROM user WHERE registration_date > 100 GROUP BY mail;",
    ...
    def test_wrong_order_by(self):
        self._query_test_case("SELECT username, mail, password FROM user WHERE registration_date > 100 ORDER BY registration_date;",
    ...
    def test_wrong_string(self):
        self._query_test_case("SELECT username, mail, password FROM user WHERE username = 'tester';",
...
class QueryProcessorComplexSelTest(QueryProcessorQueryTestBast):
    def test_count(self):
        self._query_test_case("SELECT registration_date, COUNT(username) FROM user WHERE registration_date > 100 GROUP BY registration_date;",
                              "SELECT registration_data, COUNT(username) FROM user WHERE registration_date > 100 GROUP BY registration_date;",
                              should_pro_attributes_right=False)

    def test_distinct_count(self):
        self._query_test_case("SELECT registration_date, COUNT(DESTINCT username) FROM user WHERE registration_date > 100 GROUP BY registration_date;",
...
class QueryProcessorComplexWhereTest(QueryProcessorQueryTestBast):
    def test_wrong_name_with_bool_op(self):
        self._query_test_case("SELECT username, mail, password FROM user WHERE registration_date > 100 AND mail LIKE '%gmail.com';",
