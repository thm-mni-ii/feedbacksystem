...
def _populate_db(att_list, cursor):
    ...
    for i in range(2 ** bits):
        binary_number = format(i, f'0{bits}b')  # Convert the current number to a binary string
        decimal_number = int(binary_number, 2)  # Convert the binary string to a decimal number
        offset = -1  # Initialize offset for iterating through the binary number from right to left
