...
class SolutionFetcher(metaclass=ABCMeta):
    @abstractmethod
    def fetch_solution(self, task_id: int, submission: Submission) -> Tuple[Result, float]:
        pass
...
class FirstSolutionFetcher(SolutionFetcher):
    ...
    def fetch_solution(self, task_id: int, submission: Submission) -> Tuple[Result, float]:
        return Result.from_db_dict(self._datastore.query("Queries", {"taskId": task_id, "passed": True})), 1.0
...
class NearestSolutionFetcher(SolutionFetcher):
    ...
    def fetch_solution(self, task_id: int, submission: Submission) -> Tuple[Result, float]:
        solutions = self._datastore.query("Queries", {"taskId": task_id, "passed": True})
        solutions = [Result.from_db_dict(solution) for solution in solutions]
        min_distance = math.inf
        min_solution = None
        for solution in solutions:
            distance = self._distance_calculator.calculate_distance(solution.statement, submission.submission)
