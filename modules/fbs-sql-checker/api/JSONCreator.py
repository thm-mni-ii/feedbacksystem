...
def checkSolutionChars(
    ...
    for x in mycol.find({"taskNumber": taskNr}):
        ...
        for y in mycol.find({"id": id}, {"havingAttribute": 1}):
            having.append(y["havingAttribute"])
...
def insertTables(mydb, elem, my_uuid, client):
    ...
    if len(extractProAttributes(elem["submission"], client)) == 1:
        ...
        record = jsonProAttribute(
            my_uuid, extractProAttributes(elem["submission"], client)[0]
        )
        mycollection.insert_one(record)
    ...
    if len(AWC.extractHaving(elem["submission"], client)) == 1:
        ...
        record = jsonHavingAttribute(
            my_uuid, AWC.extractHaving(elem["submission"], client)[0]
        )
        mycollection.insert_one(record)
    elif len(AWC.extractHaving(elem["submission"], client)) > 1 and not isinstance(
        AWC.extractHaving(elem["submission"], client), str
    ):
        mycollection = mydb["Having"]
