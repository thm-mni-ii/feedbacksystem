    ...
    for i, token in enumerate(tokens):
        if isinstance(token, sqlparse.sql.Token):
            if token.ttype == sqlparse.tokens.Keyword and token.value == c.FROM:
                if isinstance(tokens[i + 2], sqlparse.sql.Identifier):
                    _extract_table_elements(tokens[i + 2], tab_map, name_list)
                elif isinstance(tokens[i + 2], sqlparse.sql.IdentifierList):
                    for t in list[sqlparse.sql.Identifier](tokens[i + 2].get_identifiers()):
            ...
            if token.ttype == sqlparse.tokens.Keyword and token.value == c.ON:
                if isinstance(tokens[i + 2], sqlparse.sql.Comparison):
                    comp_list.append(f.format_comp_db_name(tokens[i + 2].value))
            if isinstance(token, sqlparse.sql.Where):
                print(f"Where tokens --------------------------------\n{token.tokens}")
                for t in token.tokens:
                    if isinstance(t, sqlparse.sql.Comparison):
                        comp_list.append(f.format_comp_db_name(t.value))
                join_list.append(token.token_first().value)  # append keyword where
