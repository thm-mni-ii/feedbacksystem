  ...
  updateScheme$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(([, activeDb]) => {
        ...
        if (!token || !token.id) return of(SqlPlaygroundActions.updateSchemeFailure({ error: 'No token' }));
        if (!activeDb) return of(SqlPlaygroundActions.updateSchemeFailure({ error: 'No active database selected' }));
  ...
  submitStatement$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(([{ statement }, activeDb]) => {
        const token = this.authService.getToken();
        if (!token) return of(SqlPlaygroundActions.submitStatementFailure({ error: 'No token' }));
        if (!activeDb) {
          this.snackbar.open("Keine Datenbank ausgewählt", "Fehler", {
            duration: 3000,
          });
          return of(SqlPlaygroundActions.submitStatementFailure({ error: 'No active database selected' }));
        }
          ...
          .pipe(
            mergeMap((result) =>
              timer(300, 500).pipe(
                ...
                catchError((error) =>
                  of(SqlPlaygroundActions.submitStatementFailure({ error: error?.message ?? 'Fehler beim Abrufen der Ergebnisse' }))
                )
              )
            ),
            catchError((error) =>
              of(SqlPlaygroundActions.submitStatementFailure({ error: error?.message ?? 'Fehler beim Ausführen der SQL-Anweisung' }))
            )
          );
      })
    )
