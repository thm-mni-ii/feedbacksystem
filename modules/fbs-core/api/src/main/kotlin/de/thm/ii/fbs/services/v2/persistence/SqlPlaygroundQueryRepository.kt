...
interface SqlPlaygroundQueryRepository : JpaRepository<SqlPlaygroundQuery, Int> {
    ...
    @Query("SELECT * FROM fbs.sql_playground_query WHERE creator_id_user_id = ?1 AND run_in_id = ?2 AND id = ?3", nativeQuery = true)
    fun findByCreatorIdAndRunInidAndId(userId: Int, databaseId: Int, queryId: Int): SqlPlaygroundQuery?
    @Query("SELECT * FROM fbs.sql_playground_query WHERE creator_id_user_id = ?1 AND run_in_id = ?2", nativeQuery = true)
