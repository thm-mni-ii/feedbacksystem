...
import de.thm.ii.fbs.verticles.runner.SqlPlaygroundShareVerticle.RUN_ADDRESS
 ...
 */
object SqlPlaygroundShareVerticle {
  /** Event Bus Address to start an runner */
  val RUN_ADDRESS = "de.thm.ii.fbs.runner.sqlPlayground.share"
}

/**
 * Verticle that starts the SqlPlayground
 */
class SqlPlaygroundShareVerticle extends ScalaVerticle {
  private val logger = ScalaLogger.getLogger(this.getClass.getName)
  private var sqlPools = Map[String, SqlPoolWithConfig]()
  private val meter = Metrics.openTelemetry.meterBuilder("de.thm.mni.ii.fbs.verticles.runner.playground.share").build()
  ...
  override def startFuture(): Future[_] = {
    val psqlDataSource = s"${PSQL_CONFIG_KEY}_playground_share"
    val psqlConfig = new JsonObject()
      .put("user", config.getString("SQL_PLAYGROUND_SHARE_PSQL_SERVER_USERNAME", "root"))
      .put("password", config.getString("SQL_PLAYGROUND_SHARE_PSQL_SERVER_PASSWORD", ""))
      .put("url", config.getString("SQL_PLAYGROUND_SHARE_PSQL_SERVER_URL", "jdbc:postgresql://localhost:5432"))
    ...
    vertx.eventBus().consumer(RUN_ADDRESS, sqlPlaygroundShare).completionFuture()
  }
  ...
  private def copyDatabaseAndCreateUser(runArgs: SqlPlaygroundShareArgs): Future[Unit] = {
    val sourceURI = buildSourceUrl(s"playground_db_${runArgs.database.id}")
    new PsqlNativeOperationsService(sourceURI).dump().flatMap(dump => {
      val ops = new PsqlOperationsService(
        runArgs.id,
        config.getString("SQL_PLAYGROUND_SHARE_PSQL_SERVER_USERNAME", "root"),
  ...
  private def sqlPlaygroundShare(msg: Message[JsonObject]): Future[Unit] = Future {
